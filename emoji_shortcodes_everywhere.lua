-- Made by wavecat with :red_heart:

--[[

Emoji shortcodes everywhere (=0.6.1)
...
local function watcher(input)
    ...
    if text ~= nil and text.toString ~= nil then
        text = text:toString()
        local results = text:gsub(
                "(:([a-zA-Z_%-]*):)",
                function(expression, shortcode)
                    return shortcodes[shortcode] or expression
                end
        )
        if results ~= text then
            local shift = utf8.len(text) - utf8.len(results)
            inline:setText(input, results)
            inline:setSelection(input, input:getTextSelectionStart() - shift, input:getTextSelectionEnd() - shift)
        end
    end
end
